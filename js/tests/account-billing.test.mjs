...
async function fixture(t) {
  ...
  const billing = createAccountBilling({ accounts, config, enabled: true,
    ...
    fetchImpl: async (url, options) => {
      ...
      if (String(url).includes('/portal/')) return Response.json({ link: 'https://customers.freemius.com/secure-example' });
...
async function recoveryFixture(t, beforeCheckout = async () => {}) {
  ...
  return { ...f, started, state,
    ...
    signedRedirect: () => {
      const raw = `${origin}/app?checkout_state=${state}&license_id=101&user_id=202&plan_id=55641`;
...
test('portal requires a linked purchase and sends no buyer identity or auto-login token', async t => {
  const f = await fixture(t);
  assert.equal((await f.call('/api/account/portal')).status, 409);
  await f.call('/api/account/restore', { licenseKey: canonical.secret_key });
  const apiCalls = f.calls.length;
  f.outage(true);
  const result = await f.call('/api/account/portal');
  assert.equal(result.status, 200);
  assert.deepEqual(result.body, { ok: true, url: 'https://customers.freemius.com/login/' });
  assert.equal(f.calls.length, apiCalls, 'opening the portal must not create a provider login or retrieve buyer data');
});
