  ...
  try {
    {
      ...
      assert.ok(!requests.some(path => path.startsWith('/api/') && !['/api/pricing', '/api/account/config', '/api/account/session'].includes(path)),
        'Landing may read pricing and session identity, while checkout and reports stay lazy');
    ...
    }
    {
      ...
      await page.waitForURL(acceptedUrl);
