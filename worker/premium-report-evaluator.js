...
const metric = (key, label, value, unit = '', rating = 'info') => ({
  key,
  label,
  value: value ?? null,
  unit,
...
const rate = (val, good, warn) => {
  if (val === null || val === undefined) return 'info';
  return val >= good ? 'good' : val >= warn ? 'fair' : 'poor';
};

const rateReverse = (val, good, warn) => {
  if (val === null || val === undefined) return 'info';
  return val <= good ? 'good' : val <= warn ? 'fair' : 'poor';
};

const percent = (rateValue) => rateValue === null || rateValue === undefined ? null : +(rateValue * 100).toFixed(1);
...
function buildBitrateMetrics(report) {
  ...
  return [
    ...
    metric('bitrateDeviation', 'Bitrate Drift', deviationPct, '%', rateReverse(Math.abs(source.bitrateDeviation ?? 0), 0.1, QUALITY.BITRATE_DEVIATION_WARNING))
  ];
}
...
const RULE_GROUPS = [
  analyzeConstraintCorrelation,
  analyzeProfileSpecific,
  analyzeSystemSignals,
  analyzeSettingSignatures
];
...
function normalizeRec(r) {
  const category = r.category || (r.type === 'profile' ? 'profile' : 'setting');
...
export function evaluatePremiumReport(report) {
